...
func (c *Client) declareTopology(ch *amqp.Channel) error {
	...
	// This service consumes the full catalog of event/ticket/venue events on
	// its own queue and rebuilds its Elasticsearch indices in response.
	if _, err := ch.QueueDeclare(QueueSearchSync, true, false, false, false, nil); err != nil {
		return err
	}
	keys := []string{
		KeyEventCreated, KeyEventUpdated, KeyEventDeleted, KeyEventPublished,
		KeyTicketTypeCreated, KeyTicketTypeUpdated, KeyTicketPurchased, KeyTicketSaleReleased,
		KeyVenueCreated, KeyVenueUpdated, KeyVenueDeleted,
	}
	for _, k := range keys {
		if err := ch.QueueBind(QueueSearchSync, k, ExchangeDonjoEvents, false, nil); err != nil {
			return err
		}
	}
	return nil
