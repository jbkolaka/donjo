...
func (c *Client) declareTopology(ch *amqp.Channel) error {
	...
	if _, err := ch.QueueDeclare(QueueNotificationSync, true, false, false, false, nil); err != nil {
		return err
	}
	keys := []string{
	...
	for _, k := range keys {
		if err := ch.QueueBind(QueueNotificationSync, k, ExchangeDonjoEvents, false, nil); err != nil {
			return err
		}
	}
	return nil
...
// Publish publishes an envelope to the donjo.events topic exchange (best-effort).
