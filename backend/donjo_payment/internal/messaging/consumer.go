...
// StartConsumers registers the handler for queue QueuePaymentSales. It is
...
func (c *Client) StartConsumers(ctx context.Context, onPurchase PurchaseHandler) {
	c.consumeCtx = ctx
	c.RegisterConsumer(QueuePaymentSales, func(env Envelope) error {
		return c.handle(env, onPurchase)
	})
}

func (c *Client) handle(env Envelope, onPurchase PurchaseHandler) error {
	if env.Key != KeyTicketPurchased || onPurchase == nil {
		return nil
	}
	...
	if p.OrderID == "" || p.TicketID == "" || p.UserID == "" || p.Quantity <= 0 {
