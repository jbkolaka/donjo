...
func (c *Client) declareTopology(ch *amqp.Channel) error {
	...
	// This service consumes completed purchases on its own queue and publishes
	// back payment.processed on the shared exchange.
	if _, err := ch.QueueDeclare(QueuePaymentSales, true, false, false, false, nil); err != nil {
		return err
	}
	return ch.QueueBind(QueuePaymentSales, KeyTicketPurchased, ExchangeDonjoEvents, false, nil)
}
