...
func (c *Client) declareTopology(ch *amqp.Channel) error {
	...
	// This service consumes completed purchases and payment records; it
	// declares only its own queue instead of relying on another service's
	// topology.
	if _, err := ch.QueueDeclare(QueueBookingSales, true, false, false, false, nil); err != nil {
		return err
	}
	for _, key := range []string{KeyTicketPurchased, KeyPaymentProcessed} {
		if err := ch.QueueBind(QueueBookingSales, key, ExchangeDonjoEvents, false, nil); err != nil {
			return err
		}
	}
	return nil
